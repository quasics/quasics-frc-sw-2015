...
public class DrivebaseBase extends SubsystemBase implements IDrivebasePlus {
   ...
   *
   * Note that this will also call "setInverted()" on the left/right
   * controllers, based on the data in config.
   *
   * @param config          drive configuration
   * @param leftController  motor controller for the left side of the drivebase
   * @param rightController motor controller for the right side of the drivebase
  ...
  protected DrivebaseBase(DriveConfig config,
    ...
    m_feedforward = new SimpleMotorFeedforward(
        m_config.feedForward().linear().kS().in(Volts),
    ...
    m_leftPID = new PIDController(m_config.leftPid().kP(),
        m_config.leftPid().kI(), m_config.leftPid().kD());

    /** PID controller for right side velocity control. */
    m_rightPID = new PIDController(m_config.rightPid().kP(),
        m_config.rightPid().kI(), m_config.rightPid().kD());
  }
  ...
  public void driveArcade(LinearVelocity speed, AngularVelocity rotation) {
    ...
    final DifferentialDriveWheelSpeeds wheelSpeeds = m_kinematics.toWheelSpeeds(
        new ChassisSpeeds(speed, ZERO_MPS, rotation));
  ...
  public void periodic() {
    m_odometry.update(m_rawGyro.getRotation2d(),
        new DifferentialDriveWheelPositions(m_leftTrivialEncoder.getPosition(),
  ...
  public void driveTankWithPID(ChassisSpeeds speeds) {
    ...
    final double rightOutput = m_rightPID.calculate(
        m_rightTrivialEncoder.getVelocity().in(MetersPerSecond),
