...
public:
   ...
   */
  VisionSettingsHelper(const std::filesystem::path& filename)
    : m_filename(filename) {}
   ...
   */
  bool SaveToFile() {
      std::ofstream fout(m_filename.native());
...
private:
  void LoadFromFile(
      ...
      std::ifstream fin(m_filename.native());
      ...
      while(fin) {
          fin >> key >> value;
          if (fin) {
            if (key == "low_h") { low_h = value; }
            if (key == "low_s") { low_s = value; }
            if (key == "low_v") { low_v = value; }
            if (key == "high_h") { high_h = value; }
            if (key == "high_s") { high_s = value; }
            if (key == "high_v") { high_v = value; }
          }
      }
  }
