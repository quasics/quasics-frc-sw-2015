...
#ifdef USE_SPARKS_VIA_CAN
  /*All instances of rev:: have been changed to frc:: I believe this is the correct fix but I am not*/
...
Drivebase::Drivebase()
  ...
  units::meter_t wheelCircumference =
      kWheelDiameter * units::constants::pi;  // Will auto-convert to meters
  ...
  std::cout << "Wheel circumference: " << wheelCircumference.value() << "\n"
            << "Velocity adjustment: " << velocityAdjustment.value() << std::endl;
...
units::meter_t Drivebase::GetRightEncoderDistance() {
  auto distance = (GetRightEncoderCount() / kGearRatio_2021) *
                  (kWheelDiameter * units::constants::pi);
  return distance;
}
