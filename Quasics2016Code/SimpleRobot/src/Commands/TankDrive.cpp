...
const std::string kSliderForDrivePower("DB/Slider 0");
const double kDefaultSliderPower(2.5);

// Called just before this Command runs the first time
void TankDrive::Initialize() {
	SmartDashboard::PutNumber(kSliderForDrivePower, kDefaultSliderPower);
}
...
#define USE_SLIDER_FOR_POWER
	...
	const double powerAdjustment = kMinPowerSetting + sliderPercent * (kMaxPowerSetting - kMinPowerSetting);
	Robot::driveSystem->MoveLeft(
			double(Robot::oi->getPilotStick()->GetRawAxis(1)) * powerAdjustment);
	Robot::driveSystem->MoveRight(
			double(Robot::oi->getPilotStick()->GetRawAxis(3)) * powerAdjustment);
#else
