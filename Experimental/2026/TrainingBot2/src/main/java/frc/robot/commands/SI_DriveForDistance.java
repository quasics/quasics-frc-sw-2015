 ...
 */
public class SI_DriveForDistance extends Command {
  /** The subsystem we're using to drive. */
  final AbstractDrivebase m_drivebase;

  /** How fast we should be moving. */
  final double m_RpercentSpeed;
  final double m_LpercentSpeed;
  ...
  /**
   * Creates a new SI_DriveForDistance, using our *other* constructor to do the work.
   * (This will assume that we want to go at 50% speed.)
   * 
   * @param drivebase The drivebase to use.
   * @param distance  The distance to drive.
   * 
   * @see #SI_DriveForDistance(AbstractDrivebase, double, Distance)
   */
//  public SI_SI_DriveForDistance(AbstractDrivebase drivebase, Distance distance) {
...
//  }
   ...
   * @see #SI_DriveForDistance(AbstractDrivebase, double, Distance)
   */
//  public SI_SI_DriveForDistance(AbstractDrivebase drivebase, double LpercentSpeed, double RpercentSpeed, double meters) {
//    this(drivebase, LpercentSpeed, Meters.of(meters));
//  }

  /**
   * Creates a new SI_DriveForDistance.
   *
   ...
   * @param distance     The distance to drive.
   */
  public SI_DriveForDistance(AbstractDrivebase drivebase, double LpercentSpeed, double RpercentSpeed, Distance distance) {
    ...
    m_LpercentSpeed = Math.signum(rawDistance) * Math.abs(LpercentSpeed);
    m_RpercentSpeed = Math.signum(rawDistance) * Math.abs(RpercentSpeed);
  ...
  public void initialize() {
    ...
    // Start moving.
    m_drivebase.tankDrive(m_LpercentSpeed, m_RpercentSpeed);
  }
