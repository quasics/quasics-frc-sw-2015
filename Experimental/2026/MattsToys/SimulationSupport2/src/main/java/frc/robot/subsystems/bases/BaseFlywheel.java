...
public abstract class BaseFlywheel extends SubsystemBase implements IFlywheel {
  ...
  /**
   * Constructor for BaseFlywheel. Initializes the feedforward controller with
   * the given constants.
   *
  ...
  protected BaseFlywheel(SimpleFeedForwardConfig feedforwardConfig) {
    this(new SimpleMotorFeedforward(feedforwardConfig.kS().in(Volts),
        feedforwardConfig.kV().in(Volts), feedforwardConfig.kA()));
  }
   ...
   * the given constants.
   *
   * @param feedforward a SimpleMotorFeedforward object that has been
   *     initialized
   ...
   *           static friction and start the flywheel moving. For many
   * flywheels, this can often be ignored (i.e., set to 0) because once the
   *           flywheel is spinning, it doesn't require much voltage to keep it
   *           going.
   * @param kV The velocity gain (Volts per RPM). This is the voltage required
   *     to
   *           maintain a certain RPM. For example, if kV is 0.002 Volts per
   * RPM, then to maintain 3000 RPM, you would need 0.002 * 3000 = 6 Volts.
   ...
   *           0.0001 Volts per RPM/s, then to accelerate at 1000 RPM/s, you
   * would need 0.0001 * 1000 = 0.1 Volts.
   */
  ...
  @Override public abstract void setRPM(double targetRPM);

  @Override public abstract double getCurrentRPM();

  @Override public abstract double getSetpointRPM();
}
