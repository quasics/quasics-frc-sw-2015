// RobotBuilder Version: 2.0
//
// This file was generated by RobotBuilder. It contains sections of
// code that are automatically generated and assigned by robotbuilder.
// These sections will be updated in the future when you export to
// C++ from RobotBuilder. Do not put any code or make any change in
// the blocks indicating autogenerated code or it will be lost on an
// update. Deleting the comments indicating the section will prevent
// it from being updated in the future.

#ifndef _ROBOT_H
#define _ROBOT_H

#include "frc/WPILib.h"
#include "frc/commands/Command.h"
#include "frc/livewindow/LiveWindow.h"
#include "frc/commands/CommandGroup.h"

// BEGIN AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=INCLUDES
#include "Subsystems/DriveBase.h"
#include "Subsystems/Elbow.h"
#ifdef ENABLE_OLD_ELEVATOR
#include "Subsystems/Elevator.h"
#include "Subsystems/Lifter.h"
#endif // ENABLE_OLD_ELEVATOR
#include "Subsystems/HatchPanelManipulator.h"
#include "Subsystems/Lighting.h"
#include "Subsystems/Manipulator.h"
#include "Subsystems/NewElevator.h"
#include "Subsystems/NewHatchPanelManipulator.h"
#include "Subsystems/TwoBarLinkage.h"


    // END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=INCLUDES
#include "OI.h"

class Robot : public frc::TimedRobot {
 public:
  frc::Command* autonomousCommand = nullptr;
  static std::unique_ptr<OI> oi;
  frc::LiveWindow* lw = frc::LiveWindow::GetInstance();
  frc::SendableChooser<frc::Command*> chooser;
  // BEGIN AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=DECLARATIONS
	static std::shared_ptr<DriveBase> driveBase;
	static std::shared_ptr<Manipulator> manipulator;
  #ifdef ENABLE_OLD_ELEVATOR
	static std::shared_ptr<Elevator> elevator;
	static std::shared_ptr<Lifter> lifter;
  #endif // ENABLE_OLD_ELEVATOR
	static std::shared_ptr<Elbow> elbow;
	static std::shared_ptr<Lighting> lighting;
	static std::shared_ptr<TwoBarLinkage> twoBarLinkage;
	static std::shared_ptr<NewElevator> newElevator;
	static std::shared_ptr<HatchPanelManipulator> hatchPanelManipulator;
	static std::shared_ptr<NewHatchPanelManipulator> newHatchPanelManipulator;

    // END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=DECLARATIONS
  static std::shared_ptr<frc::CommandGroup> teleopCommand;

  void RobotInit() override;
  void DisabledInit() override;
  void DisabledPeriodic() override;
  void AutonomousInit() override;
  void AutonomousPeriodic() override;
  void TeleopInit() override;
  void TeleopPeriodic() override;

  enum LifterMode{
    eElevator,
    eLifter,
    eBoth
  };

//  static void SetLifterMode(LifterMode newm);
//  static LifterMode GetLifterMode();

 private:
  //static LifterMode currentLifterMode;
};
#endif
