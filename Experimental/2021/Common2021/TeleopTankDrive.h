 ...
 * 
 * This also provides support for "switch drive", where the front and rear of the
 * robot are treated as being reversed (which also means that the logical
...
class TeleopTankDrive
 ...
 public:
   ...
   * @param leftSpeedSupplier   function returning the requested speed for the left side
   * @param rightSpeedSupplier  function returning the requested speed for the right side
   * @param switchDriveEnabled  function returning "true" if "switch mode" should be active
   *                            (if not provided, switch mode is never active)
   */
  ...
  void Execute() override {
    if (m_switchDriveEnabled()) {
      ...
      m_driveBase->TankDrive(-m_rightSpeedSupplier(), -m_leftSpeedSupplier());
    } else {
