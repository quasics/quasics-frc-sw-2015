...
public class SupportFunctions {
  ...
  public static <T> boolean isBetween(T x, T a, T b) {
    if (x instanceof Distance dx && a instanceof Distance da && b instanceof Distance db) {
      return (dx.gte(da) && dx.lte(db)) || (dx.gte(db) && dx.lte(da));
    } else if (x instanceof Angle ax && a instanceof Angle aa && b instanceof Angle ab) {
      return (ax.gte(aa) && ax.lte(ab)) || (ax.gte(ab) && ax.lte(aa));
    } else {
      throw new IllegalArgumentException(
          "isBetween() does not support type " + x.getClass().getName());
    }
  }
}
