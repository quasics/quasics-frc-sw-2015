 ...
 */
public class ShooterHoodSubsystem
    extends SubsystemBase implements IShooterHood {
  /**
  ...
  public ShooterHoodSubsystem(HoodConfig hoodConfig) {
    ...
    //
    // Note that we are assuming that the absolute encoder is directly measuring
    // the hood angle, so the conversion factor is just 360 degrees per
    // rotation. If there were gearing between the motor and the hood, we would
    // need to adjust the conversion factor accordingly (e.g., if there were a
    // 2:1 gear reduction, the conversion factor would be 180 degrees per
    // rotation).
    motorConfig.absoluteEncoder.positionConversionFactor(360.0)
        .velocityConversionFactor(360.0 / 60.0);

    // Configure the PID controller for position control, using the absolute
    // encoder as the feedback sensor. We also set an output range to prevent
    // the hood from moving too fast, which could cause mechanical stress or
    // overshooting.
  ...
  public void setPosition(Angle targetAngle) {
    ...
    final double clampedPosition = MathUtil.clamp(
        positionDegrees, getMinAngle().in(Degrees), getMaxAngle().in(Degrees));
    var result = m_pidController.setSetpoint(
        clampedPosition, SparkMax.ControlType.kPosition);
    if (result != REVLibError.kOk) {
      DriverStation.reportError(
          "Failed to set hood position: " + result, false);
    }
  }
