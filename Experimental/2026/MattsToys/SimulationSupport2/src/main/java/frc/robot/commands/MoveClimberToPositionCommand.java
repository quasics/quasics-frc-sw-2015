...
public class MoveClimberToPositionCommand extends Command {
  ...
  /**
   * If true, the command will not finish until the climber has reached the
   * target position; if false, the command will finish immediately after
   ...
   *                          the move command.
   */
  public MoveClimberToPositionCommand(
      IClimber climber, IClimber.Position position, boolean waitUntilFinished) {
    m_climber = climber;
    m_position = position;
  ...
  public boolean isFinished() {
    return !m_waitUntilFinished
        || (m_climber.getCurrentState() != IClimber.State.PidControlled);
  }
}
