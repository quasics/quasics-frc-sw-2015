...
public:
  ...
  // Note that the command is "taken over" by the button, and can't be used
  // elsewhere after it's been bound to it.
  void RunCommandWhenDriverButtonIsHeld(int logitechButtonId,
                                        std::unique_ptr<frc2::Command> command)
  {
    frc2::JoystickButton(&driverJoystick, logitechButtonId).WhileTrue(new frc2::RepeatCommand(std::move(command)));
  }
  void RunCommandWhenOperatorButtonIsHeld(int buttonId,
                                          std::unique_ptr<frc2::Command> command)
  {
    frc2::JoystickButton(&operatorController, buttonId).WhileTrue(new frc2::RepeatCommand(std::move(command)));
  }
