...
OI::OI() {
  ...
  frc::SmartDashboard::PutData("New Hatch Panel Manipulator", new NewHatchPanelManipulatorControl());
...
bool OI::isHatchManipulatorSignaledOpen() {
  const double direction = operatorStick->GetRawAxis(XBox_RightYAxis);
  if (direction > 0.3) {
    return true;
  } else {
...
bool OI::isHatchManipulatorSignaledClose() {
  const double direction = operatorStick->GetRawAxis(XBox_RightYAxis);
  if (direction < -0.3) {
    return true;
  } else {
