...
//maps Right button on the Xbox Controller to moving the Elevator to Position One
...
//maps Left Button on the Xbox Controller to moving the Elevator to Position Two
...
//gets the direction of the Xbox Controller's Right joystick responsible for moving the Elbow
double OI::getElbowDirection() {
  return operatorStick->GetRawAxis(XBox_RightYAxis);
}

//tests whether the A Button on the Xbox Controller is pressed to signal the Intake
bool OI::isIntakeSignaledPositive() {  // in
  const bool buttonA = operatorStick->GetRawButton(XBox_ButtonA);
  return buttonA;
}

//tests whether the B Button on the Xbox Controller is pressed to signal the Intake
bool OI::isIntakeSignaledLowNegative() {  // out, slowly
  const bool buttonB = operatorStick->GetRawButton(XBox_ButtonB);
  return buttonB;
}

//tests whether the Y Button on the Xbox Controller is pressed to signal the Intake
...
//tests the Left Trigger and Right Trigger on the Logitech Controller to see if both are pressed for Full Speed
...
//tests the Right Trigger on the Logitech Controller to see if it is pressed for Turtle Speed
...
bool OI::isElevatorMoveUpSignaled() {
  ...
  if (false) {
    bool result = joystick < 0;
    std::cerr << "Reporting joystick up as: " << std::boolalpha << result << '\n';
    return result;
  }
...
bool OI::isElevatorMoveDownSignaled() {
  ...
  if (false) {
    bool result = joystick < 0;
    std::cerr << "Reporting joystick down as: " << std::boolalpha << result << '\n';
    return result;
  }
