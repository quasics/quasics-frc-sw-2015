...
Vision::Vision() : frc::Subsystem("Vision"), table(nullptr) {
    // BEGIN AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=DECLARATIONS

    // END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=DECLARATIONS

	// Note that we need to use whatever name has been configured in
	// the GRIP pipeline for the table.
	table = nt::NetworkTable::GetTable("GRIP/quasicsContoursReport");
 ...
 */
class Vision::BoundingRectHelper {
	public:
		int top = 0;
		int bottom = 0;
		int left = 0;
...
double Vision::boundingRatioScore(const cv::Rect& rectangle1, const cv::Rect& rectangle2, const BoundingRectHelper& bounds) {
...
double Vision::contourWidthScore(const cv::Rect& rectangle1, const cv::Rect& rectangle2, const BoundingRectHelper& bounds) {
...
double Vision::topEdgeScore(const cv::Rect& rectangle1, const cv::Rect& rectangle2, const BoundingRectHelper& bounds) {
...
double Vision::leftSpacingScore(const cv::Rect& rectangle1, const cv::Rect& rectangle2, const BoundingRectHelper& bounds) {
...
double Vision::widthRatioScore(const cv::Rect& rectangle1, const cv::Rect& rectangle2, const BoundingRectHelper& bounds) {
	return ratioToScore(rectangle1.width/rectangle2.width);
}

// The height of the two contours should match
double Vision::heightRatioScore(const cv::Rect& rectangle1, const cv::Rect& rectangle2, const BoundingRectHelper& bounds) {
	return ratioToScore(rectangle1.height/rectangle2.height);
}

void Vision::processNetworkDataFromGrip(BoundingRects& rects) {
	if (table == nullptr) {
	...
	// Build up a collection of the bounding boxes around each contour.
	std::vector<cv::Rect> contourRects;
	contourRects.reserve(centerXs.size());
	cv::Rect r;
	for (unsigned i = 0; i < centerXs.size(); ++i) {
		r.height = heights[i];
		r.width = widths[i];
		r.x = centerXs[i] - (r.width / 2);
		r.y = centerYs[i] - (r.height / 2);
		contourRects.push_back(r);
	}

	// TODO: Actually analyze the data in "contourRects" in order to figure out where our target is.
	// (This is left as an exercise for the reader.  For now, I'm just printing them out.)

	std::cout << "Got rects: ";
	for (auto r : contourRects) {
		std::cout << r << ' ';
	}
	std::cout << std::endl;
}
...
void Vision::processVisionPipeline(grip::MultipleOutputs& pipeline) {
	...
	//
	// Part 2: Use the data from GRIP to find the target.
	processNetworkDataFromGrip(boundingRects);

//	//
//	// Part 2: If we have at least 2 green contours, we might have a vision target in view
