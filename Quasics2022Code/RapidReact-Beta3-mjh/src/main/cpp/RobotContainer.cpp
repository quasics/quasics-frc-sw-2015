...
void RobotContainer::ConfigureControllerButtonBindings()
{
    ...
    //                                      std::make_unique<ExtendClimber>(&m_climber));
    RunCommandWhenOperatorButtonIsHeld(frc::XboxController::Button::kA,
                                       std::make_unique<ExtendClimber>(&m_climber));
    //   RunCommandWhenOperatorButtonIsHeld(frc::XboxController::Button::kA,
    //                                      std::make_unique<RetractClimber>(&m_climber));
    RunCommandWhenOperatorButtonIsHeld(frc::XboxController::Button::kY,
                                       std::make_unique<RetractIntake>(&m_intakeDeployment,
                                                                       RobotValues::RETRACT_INTAKE_SPEED));
    RunCommandWhenOperatorButtonIsHeld(frc::XboxController::Button::kLeftBumper,
                                       std::make_unique<RunConveyorAtSpeed>(&m_conveyor,
    ...
    RunCommandWhenOperatorButtonIsHeld(frc::XboxController::Button::kB,
                                       std::unique_ptr<frc2::Command>(ButtonShootingHighGoal())); // 0.40 Flywheel, 0.8 backroller
    RunCommandWhenOperatorButtonIsHeld(frc::XboxController::Button::kX,
                                       std::unique_ptr<frc2::Command>(ButtonShootingLowGoal()));
    ...
    RunCommandWhenDriverButtonIsHeld(
        OperatorInterface::LogitechGamePad::LEFT_TRIGGER, std::make_unique<RunIntakeAtSpeed>(&m_intake,
                                                                                             RobotValues::INTAKE_BACKWARD_SPEED));
    RunCommandWhenDriverButtonIsHeld(
        OperatorInterface::LogitechGamePad::RIGHT_TRIGGER, std::make_unique<RunIntakeAtSpeed>(&m_intake,
                                                                                              RobotValues::INTAKE_FORWARD_SPEED));
    RunCommandWhenDriverButtonIsHeld(OperatorInterface::LogitechGamePad::X_BUTTON,
                                     std::make_unique<ExtendIntake>(&m_intakeDeployment, 0.40));
}
...
frc2::ParallelRaceGroup *RobotContainer::ButtonShootingHighGoal()
{
    ...
    commands.push_back(
        std::unique_ptr<frc2::Command>(ConveyorDelay()));
...
{
    std::vector<std::unique_ptr<frc2::Command>> commands;
    commands.push_back(std::unique_ptr<frc2::Command>(GenerateBallShootingSequence(1)));
    commands.push_back(
        std::make_unique<DriveAtPowerForMeters>(&m_drivebase, -0.35, -2.8_m));
...
{
    std::vector<std::unique_ptr<frc2::Command>> commands;
    commands.push_back(std::unique_ptr<frc2::Command>(BUCKLE()));
    commands.push_back(
    ...
    commands.push_back(
        std::unique_ptr<frc2::Command>(MoveWithRetractedIntake(2_m)));
    commands.push_back(
...
frc2::SequentialCommandGroup *RobotContainer::RSM1()
{
    ...
    commands.push_back(
        std::unique_ptr<frc2::Command>(GenerateBallShootingSequence(1)));
    commands.push_back(std::unique_ptr<frc2::Command>(Move("RSM1")));
    return new frc2::SequentialCommandGroup(std::move(commands));
}
...
frc2::SequentialCommandGroup *RobotContainer::RSM2()
{
    ...
    commands.push_back(
        std::unique_ptr<frc2::Command>(GenerateBallShootingSequence(1)));
    commands.push_back(std::unique_ptr<frc2::Command>(Move("RSM2")));
    return new frc2::SequentialCommandGroup(std::move(commands));
}
...
frc2::SequentialCommandGroup *RobotContainer::BSM3()
{
    ...
    commands.push_back(
        std::unique_ptr<frc2::Command>(GenerateBallShootingSequence(1)));
    commands.push_back(std::unique_ptr<frc2::Command>(Move("BSM3")));
    return new frc2::SequentialCommandGroup(std::move(commands));
}
...
frc2::SequentialCommandGroup *RobotContainer::BSM4()
{
    ...
    commands.push_back(
        std::unique_ptr<frc2::Command>(GenerateBallShootingSequence(1)));
    commands.push_back(std::unique_ptr<frc2::Command>(Move("BSM4")));
    return new frc2::SequentialCommandGroup(std::move(commands));
}
//
//
    ...
    std::string Path)
{
    ...
    Path = Path + ".wpilib.json";
    commands.push_back(
        std::unique_ptr<frc2::Command>(DrivingAndPickingUpBall(Path)));
    commands.push_back(
    ...
    commands.push_back(std::unique_ptr<frc2::Command>(
        m_trajectoryGenerator.GenerateCommandFromPathWeaverFile(
            Path, TrajectoryCommandGenerator::TelemetryHandling::
                      ResetTelemetryAtStart)));
    commands.push_back(
        std::unique_ptr<frc2::Command>(PickingUpBall()));
    ...
    int amountBalls)
{
    ...
    {
        commands.push_back(
            std::unique_ptr<frc2::Command>(BuildShootBallSequence()));
        count--;
    }
    return new frc2::SequentialCommandGroup(std::move(commands));
}
