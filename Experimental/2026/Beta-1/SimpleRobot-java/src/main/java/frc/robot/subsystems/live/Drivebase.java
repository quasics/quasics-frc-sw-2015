...
public class Drivebase extends AbstractDrivebase {
  ...
  private final IGyro m_wrappedGyro =
      new Pigeon2Wrapper(new Pigeon2(Constants.QuasicsDrivebaseCanIds.PIGEON2_CAN_ID));

  /** Left leading motor. */
  final private SparkMax m_leftLeader =
      new SparkMax(Constants.QuasicsDrivebaseCanIds.LEFT_LEADER_ID, MotorType.kBrushless);
  /** Right leading motor. */
  final private SparkMax m_rightLeader =
      new SparkMax(Constants.QuasicsDrivebaseCanIds.RIGHT_LEADER_ID, MotorType.kBrushless);

  /** Left encoder (as a TrivialEncoder). */
  final private TrivialEncoder m_leftEncoder =
      new SparkMaxEncoderWrapper(m_leftLeader.getEncoder());
  /** Right encoder (as a TrivialEncoder). */
  final private TrivialEncoder m_rightEncoder =
      new SparkMaxEncoderWrapper(m_rightLeader.getEncoder());
  ...
  public Drivebase(RobotConfig config) {
    ...
    configureSparkMaxEncoderForDistance(
        leftLeaderConfig, driveConfig.wheelRadius(), driveConfig.gearing());
    configureSparkMaxEncoderForDistance(
        rightLeaderConfig, driveConfig.wheelRadius(), driveConfig.gearing());
    ...
    m_poseEstimator = new DifferentialDrivePoseEstimator(m_kinematics,
        ...
        m_rightEncoder.getPosition().in(Meters), new Pose2d(),
