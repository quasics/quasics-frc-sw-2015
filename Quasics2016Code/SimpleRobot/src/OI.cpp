...
std::unique_ptr<Command> OI::GetAutoCommand () const {
	std::unique_ptr<Command> result;

	const Command* selectedCommand = (Command*)chooser->GetSelected();
	if (selectedCommand == nullptr) {
		cout << "Error: Can't get autonomous command from the chooser!" << endl;
	} else {
		const CloneableCommand* cloneable = dynamic_cast<const CloneableCommand*>(selectedCommand);
		if (cloneable == nullptr) {
			cout << "Error: Selected autonomous command isn't cloneable!" << endl;
		} else {
