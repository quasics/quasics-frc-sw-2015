...
//If distance or speed is negative, then we assume were going backward for abs(distance) meters
DriveUntilPitchAngleChange::DriveUntilPitchAngleChange(Drivebase *drivebase, double power, units::meter_t distance) : m_drivebase(drivebase), 
  m_power((power < 0 || distance.value() < 0) ? -std::abs(power) : std::abs(power)), m_distance(std::abs(distance.value()))
{
